...
class TestProducts:
    ...
    def test_product_buy_more_than_available(self, product):  # Положительная проверка. Ожидаем ValueError, если хотим купить больше, чем есть на складе
        # TODO напишите проверки на метод buy,
        #  которые ожидают ошибку ValueError при попытке купить больше, чем есть в наличии
        with pytest.raises(ValueError) as exc_info:
            product.buy(1001)  # на складе 1000, пытаемся купить 1001
...
class TestCart:
    ...
    def test_add_product_in_basket(self, product, product_basket):  # положительная проверка добавления продукта в корзину
        assert product in product_basket.add_product(product, 2)

    def test_add_2_same_product_in_basket(self, product, product_basket):  # положительная проверка того, что если в корзине уже есть продукт, то
    ...
    def test_incorrect_type_buy_count(self, product, product_basket):  # негативная проверка: возврат текста ошибки при передаче
    ...
    def test_remove_all_products_in_cart(self, product, product_basket):  # положительная проверка на полное удаление всех товаров из корзины
    ...
    def test_remove_some_products_in_cart(self, product, product_basket):  # положительная проверка удаления нескольких товаров в корзине
    ...
    def test_remove_as_many_products_as_there_are_in_the_basket(self, product, product_basket):  # положительная проверка удаления столько товара, сколько есть в корзине
        product_basket.add_product(product, 6)
        product_basket.remove_product(product, 6)
        assert product_basket.products == {}

    def test_remove_products_more_than_available_in_cart(self, product, product_basket):  # положительная проверка на удаление продуктов больше, чем есть в корзине
        product_basket.add_product(product, 6)
        product_basket.remove_product(product, 8)
        assert product_basket.products == {}
    ...
    def test_buy_product_less_than_in_stock(self, product, product_basket): # положительная проверка: исключение при попытке купить товаров больше, чем есть на складе
        product_basket.add_product(Product("book", 800, "This is a book", 10), 15)
        with pytest.raises(ValueError) as exc_info:
            product_basket.buy()
        assert "Недостаточно товара на складе" in str(exc_info.value)
